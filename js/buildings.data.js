(function (Game) {
    "use strict";
    Game.buildingsD = [
        // buildings
        {
            name:"Incandescent Lightbulb",
            commonName: "incandescent lightbulb|0|shining",
            cost:15,
            vps:0.1,
            description:"The greatest invasion in history has now begun.",
            displayAt: -1,
            onBuy: function (Game) {
                if (this.amount > 0) Game.earnAchievement("inc1");
                if (this.amount > 2) Game.earnAchievement("inc2");
                if (this.amount > 50) Game.earnAchievement("inc3");
                if (this.amount > 100) Game.earnAchievement("inc4");
            }
        },
        {
            name:"Lightbulb Smasher",
            commonName: "lightbulb smasher|0|smashing",
            cost:100,
            vps:0.5,
            description:"Hires some muscular guys to smash bulbs.",
            displayAt: 40,
            onBuy: function (Game) {
                if (this.amount > 0) Game.earnAchievement("smash1");
                if (this.amount > 50) Game.earnAchievement("smash2");
            }
        },
        {
            name:"Thunder Collector",
            commonName: "lightbulb smasher|0|collecting",
            cost:500,
            vps:4,
            description:"Collects thunder every where they strike.",
            displayAt: 125
        },
        {
            name:"Halogen Lightbulb",
            commonName: "halogen lightbulb|0|shining",
            cost:3141,
            vps:15,
            description:"AHH! IT's TO BRIGHT!",
            displayAt: 300
        },
        {
            name:"Tan Lightbulb",
            commonName: "tan lightbulb|0|shining",
            cost:10000,
            vps:40,
            description:"much better...",
            displayAt: 1000,
        },
        {
            name:"LED Lightbulb",
            commonName: "LED lightbulb|0|shining",
            cost:40000,
            vps:100,
            description:"LEDs you to your destiny.",
            displayAt: 15000,
        },
        {
            name:"Bacterial Lightbulb",
            commonName: "bacterial lightbulb|0|producing",
            cost:200000,
            vps:400,
            description:"Small, tiny life forms that will soon take over the WORLD!",
            displayAt: 99999,
        },
        {
            name:"Dinosaur Lightbulb",
            commonName: "dinosaur lightbulb|0|shining",
            cost:1618033,
            vps:161803,
            description:"Jurassic Park remake CONFIRMED!... or is it?",
            displayAt:19930
        },
        {
            name:"Human Lightbulb",
            commonName: "human lightbulb|0|shining",
            cost:100110001,
            vps:99999,
            description:"Every human will now obey your commmands",
            displayAt:871995
        },
    ];
})(window.Game || {});